...
#include "../core/agri3d_config.h"
#include "../core/agri3d_state.h"
...
static void performWeedAction(uint8_t clientNum, float captureX, float captureY,
    ...
    // ── Run AI Inference ────────────────────────────────────────────────────
    AiResult res = aiAnalyzeFrame((uint8_t*)jpegBuf, jpegLen);

    if (res.foundWeed) {
        // Calculate absolute weed coordinates
        // Note: xOffset/yOffset are in pixels. Assuming 1px = X mm (calibration needed)
        // For now we just report the detection.
        float wx = captureX; // + offset calculation if needed
        float wy = captureY;

        AgriLog(TAG_WEED, LEVEL_WARN, "WEED DETECTED @ (%.1f, %.1f) conf=%.2f", 
                      wx, wy, res.confidence);

        StaticJsonDocument<128> doc;
        doc["evt"]        = "WEED_DETECTED";
        doc["x"]          = wx;
        doc["y"]          = wy;
        doc["confidence"] = res.confidence;
        String out; serializeJson(doc, out);
        webSocket.sendTXT(clientNum, out);

        // TODO: Actuate weed removal tool (e.g. laser or mechanical)
        // enqueueGrblCommand("M105"); 
    }
}
...
void handleAutoDetectPlants(uint8_t clientNum, const String& params) {
    ...
    for (int row = 0; row < rows; row++) {
        for (int colStep = 0; colStep < cols; colStep++) {
            ...
            // ── AI Plant Detection ──────────────────────────────────────────
            AiResult res = aiAnalyzeFrame(fb->buf, fb->len);
            bool  isPlant    = res.foundPlant;
