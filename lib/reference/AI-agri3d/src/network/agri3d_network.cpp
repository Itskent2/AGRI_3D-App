...
static esp_err_t stream_handler(httpd_req_t *req) {
    ...
    while(true) {
        ...
        if (!fb) {
            AgriLog(TAG_NET, LEVEL_ERR, "Camera capture failed");
            res = ESP_FAIL;
        } else {
            _jpg_buf_len = fb->len;
            _jpg_buf = fb->buf;
...
static esp_err_t index_handler(httpd_req_t *req) {
    const char* html = "<html><head><title>AGRI-3D Camera</title>"
                       "<style>body{background:#111;color:#eee;text-align:center;font-family:sans-serif;}"
                       "img{max-width:100%;border:2px solid #444;border-radius:8px;margin-top:20px;}</style></head>"
                       "<body><h1>AGRI-3D Live Feed</h1>"
                       "<img src='/stream'/><br><p>Status: Streaming Active</p></body></html>";
...
void startStreamServer() {
    ...
    config.ctrl_port = 32769; // Offset from default 32768 to avoid conflict if both servers run
    ...
    if (httpd_start(&stream_httpd, &config) == ESP_OK) {
        ...
        AgriLog(TAG_NET, LEVEL_SUCCESS, "MJPEG Stream Server started on port 81");
    }
}
