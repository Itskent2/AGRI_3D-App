...
void webSocketEvent(uint8_t num, WStype_t type,
    ...
    if (cmd == "START_AI_TEST") {
        sysState.setAiTesting(true);
        AgriLog(TAG_CMD, LEVEL_INFO, "Real-time AI testing ENABLED");
        return;
    }
    if (cmd == "STOP_AI_TEST") {
        sysState.setAiTesting(false);
        AgriLog(TAG_CMD, LEVEL_INFO, "Real-time AI testing DISABLED");
        return;
    }
