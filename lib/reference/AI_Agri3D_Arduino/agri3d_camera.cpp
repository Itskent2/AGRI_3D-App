 ...
 * @brief Camera init, FPM stream task, and captureFrameAtPosition() implementation.
 */
...
bool cameraInit() {
    ...
    if (psramFound()) {
        ...
        // Fall back to SVGA if no PSRAM
        config.frame_size   = FRAMESIZE_SVGA;
        config.jpeg_quality = 12;
        config.fb_count     = 1;
        config.fb_location  = CAMERA_FB_IN_DRAM;
        config.grab_mode    = CAMERA_GRAB_WHEN_EMPTY;
        Serial.println("[CAM] WARNING: No PSRAM — falling back to SVGA");
    }
    ...
    // Launch stream task on Core 1
    xTaskCreatePinnedToCore(streamTask, "streamTask", 8192, nullptr, 2, nullptr, 1);
    return true;
}
    ...
    while (true) {
        ...
        if (sysState.isStreaming &&
            ...
            if (activeClientNum >= 0) {
                webSocket.sendBIN((uint8_t)activeClientNum, fb->buf, fb->len);
            }
...
bool captureFrameAtPosition(uint8_t clientNum, int idx, int total,
    ...
    char sdPath[96] = {0};
#if HW_SD_CONNECTED
    sdSaveImage(fb->buf, fb->len, SD_IMG_PLANTMAP, 'f',
                idx, sysState.grblX, sysState.grblY, now, sdPath);
#endif
