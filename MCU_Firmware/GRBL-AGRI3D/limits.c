...
void limits_go_home(uint8_t cycle_mask) {
  ...
  // Ensure all Agri3D relays (Water, Fertigation, Weeder) are turned off during homing for safety.
  ...
  for (idx = 0; idx < N_AXIS; idx++) {
    ...
    if (bit_istrue(cycle_mask, bit(idx))) {
      // Set target based on max_travel setting. Ensure homing switches engaged
      // with search scalar. NOTE: settings.max_travel[] is stored as a negative
      // value.
      max_travel = max(max_travel,
                       (-HOMING_AXIS_SEARCH_SCALAR) * settings.max_travel[idx]);
    }
  }

  // Set search mode with approach at seek rate to quickly engage the specified
  // cycle_mask limit switches.
  bool approach = true;
    ...
    do {
      ...
      if (sys_rt_exec_state &
        ...
        if (!approach && (limits_get_state() & cycle_mask)) {
          printPgmString(PSTR("[MSG:ALARM 8 - Pull-off Failed. Switch still engaged!]\r\n"));
          system_set_exec_alarm(EXEC_ALARM_HOMING_FAIL_PULLOFF);
        }
        // Homing failure condition: Limit switch not found during approach.
        if (approach && (rt_exec & EXEC_CYCLE_STOP)) {
          printPgmString(PSTR("[MSG:ALARM 9 - Approach Failed. Switch never triggered!]\r\n"));
  ...
  // The active cycle axes should now be homed and machine limits have been
  // located. By default, Grbl defines machine space as all negative, as do most
  // CNCs. Since limit switches can be on either side of an axes, check and set
  // axes machine zero appropriately. Also, set up pull-off maneuver from axes
  // limit switches that have been homed. This provides some initial clearance
  // off the switches and should also help prevent them from falsely triggering
  // when hard limits are enabled or when more than one axes shares a limit pin.
  ...
  // =========================================================================
  if (cycle_mask & (bit(X_AXIS) | bit(Y_AXIS))) {
    float target[N_AXIS];
    system_convert_array_steps_to_mpos(target, sys_position);
    ...
    for (idx = 0; idx < N_AXIS; idx++) {
      start_pos[idx] = sys_position[idx];
      if ((cycle_mask & bit(idx)) && (idx == X_AXIS || idx == Y_AXIS)) {
        // Drive to the opposite end (10 meters absolute maximum safety distance
        // to guarantee StallGuard triggers)
        if (bit_istrue(settings.homing_dir_mask, bit(idx))) {
          target[idx] -=
              10000.0; // Origin was positive, drive negative to find Minimum
        } else {
          target[idx] +=
              10000.0; // Origin was negative, drive positive to find Maximum
        }
    ...
    // Execute the seek motion towards the opposite bounds
    ...
    do {
      ...
      if (sys_rt_exec_state &
        ...
        // Failsafe: if the massive 10,000mm seek move finishes completely
        // without triggering StallGuard, trigger a hard limit alarm!
        if (sys_rt_exec_state & EXEC_CYCLE_STOP) {
          printPgmString(PSTR("[MSG:Auto-Dim Failed - Switch never triggered!]\r\n"));
    ...
    // Calculate and automatically save new dimensions to EEPROM!
    for (idx = 0; idx < N_AXIS; idx++) {
      if ((cycle_mask & bit(idx)) && (idx == X_AXIS || idx == Y_AXIS)) {

        // Calculate absolute step count traveled from the origin to the
        // opposite boundary
        ...
        // Subtract single pulloff distance to establish workable bounds
        measured_length -= settings.homing_pulloff;

        if (measured_length > 0) {
          // Write parameter ($130 for X, $131 for Y) direct to EEPROM
          // settings_store_global_setting internally translates 130 -> Max
          // Travel
          settings_store_global_setting(130 + idx, measured_length);
        }

        // Set machine position accurately reflecting the new physical location
        // relative to global workspace
        if (bit_istrue(settings.homing_dir_mask, bit(idx))) {
          // Hit Minimum Bound. Set coordinate accurately to -pulloff (safe work
          // area origin)
          sys_position[idx] =
              lround(-settings.homing_pulloff * settings.steps_per_mm[idx]);
        } else {
          // Hit Maximum Bound. Set coordinate accurately to Max dimensions +
          // pulloff
          sys_position[idx] =
              lround((settings.max_travel[idx] + settings.homing_pulloff) *
                     settings.steps_per_mm[idx]);
    ...
    // Perform final Pull-off maneuver from the opposite switch to prevent hard
    // limits from locking the system down
    system_convert_array_steps_to_mpos(target, sys_position);
    measure_axislock = 0;
    for (idx = 0; idx < N_AXIS; idx++) {
      if ((cycle_mask & bit(idx)) && (idx == X_AXIS || idx == Y_AXIS)) {
        if (bit_istrue(settings.homing_dir_mask, bit(idx))) {
          target[idx] += settings.homing_pulloff; // Pull off towards positive
        } else {
          target[idx] -= settings.homing_pulloff; // Pull off towards negative
        }
        measure_axislock |= get_step_pin_mask(idx);
      }
    }
    ...
    do {
      st_prep_buffer();
    } while (STEP_MASK & sys.homing_axis_lock);

    // Set absolute zero mapping correctly after pull off!
    for (idx = 0; idx < N_AXIS; idx++) {
      if ((cycle_mask & bit(idx)) && (idx == X_AXIS || idx == Y_AXIS)) {
        if (bit_istrue(settings.homing_dir_mask, bit(idx))) {
          sys_position[idx] = 0; // Exactly at Absolute Zero (Min Bound)
        } else {
          sys_position[idx] =
              lround(settings.max_travel[idx] *
                     settings.steps_per_mm[idx]); // Exactly at Absolute Max
        }
      }
    }
  }
